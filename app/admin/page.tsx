              ...
              {latest ? (
                <div className="space-y-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Link href="/admin/assets" className={cn(buttonVariants({ variant: "outline" }))}>
                      Inspect assets
                    </Link>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          <Card className="h-fit">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Focus areas for the current build.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {[
                {
                  icon: "🔄",
                  text: "Clean review loops, status badges, and a fast path to the reel detail page.",
                },
                {
                  icon: "🗂️",
                  text: "Assets are grouped by reel so image, voice, and transcript status stays easy to scan.",
                },
                {
                  icon: "🗑️",
                  text: "The delete action is wired to a real API route and prompts before removal.",
                },
                {
                  icon: "📊",
                  text: "Review scores and approval status are updated live from AI evaluation results.",
                },
              ].map((note, i) => (
                <div
                  key={i}
          ...
          <Card className="h-fit">
            ...
            <CardContent className="grid grid-cols-1 gap-2">
              {[
                { href: "/admin/reels", label: "Manage Reels", icon: Film, desc: "View & control all reels" },
                { href: "/admin/assets", label: "Browse Assets", icon: ImageIcon, desc: "Images, voice & video" },
              ].map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="group flex items-center gap-3 rounded-xl border border-border/60 p-4 transition-all duration-150 hover:border-primary/20 hover:bg-primary/5"
                  >
                    <div className="grid size-10 place-items-center rounded-lg bg-primary/10 text-primary ring-1 ring-primary/20 transition-transform duration-200 group-hover:scale-110">
